...
import { IONFT721URIStorage, MessagingFee, MessagingReceipt, SendParam } from "./interfaces/IONFT721URIStorage.sol";
import { ONFT721URIStorageMsgCodec } from "./libs/ONFT721URIStorageMsgCodec.sol";
 ...
 */
abstract contract ONFT721URIStorageCore is IONFT721URIStorage, OApp, OAppPreCrimeSimulator, OAppOptionsType3 {
    using ONFT721URIStorageMsgCodec for bytes;
    using ONFT721URIStorageMsgCodec for bytes32;
    ...
    function onftVersion() external pure virtual returns (bytes4 interfaceId, uint64 version) {
        return (type(IONFT721URIStorage).interfaceId, 1);
    }
    ...
    function _buildMsgAndOptions(
        ...
        bool hasCompose;
        (message, hasCompose) = ONFT721URIStorageMsgCodec.encode(_sendParam.to, _sendParam.tokenId, string(_sendParam.tokenURI), _sendParam.composeMsg);
    ...
    function _lzReceive(
        ...
        string memory tokenURI = _message.tokenURI();

        _credit(toAddress, tokenId, tokenURI, _origin.srcEid);
    ...
    function _credit(address /*_to*/, uint256 /*_tokenId*/, string memory /*_tokenURI*/, uint32 /*_srcEid*/) internal virtual;
}
